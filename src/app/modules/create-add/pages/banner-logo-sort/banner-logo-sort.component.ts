...
export class BannerLogoSortComponent implements OnInit {
  ...
  TypePlaceChange($event: MatSelectChange): void {
    if ($event.value === 'ships' || this.isMain==1) {
      return;
    }
    this.selectedSection = '';
    this.selectedSubSection = '';
    const sub = this.isMain === 1 ? 0 : 1;
    this.AllSections = this.appService.GetSections($event.value, this.isMain, sub).pipe(map((data) => {
  ...
  GotoForm(isShips: boolean = false): void {
    if (isShips) {
      ...
      if (this.isMain === 1) {
        ...
        if (this.selectedPlace === 'store' || this.selectedPlace === 'news' || this.selectedPlace === 'shows' || this.selectedPlace === 'tenders') {
          this.GoTo({
            company_id: this.company,
            type: this.type,
            type_place: this.selectedPlace,
