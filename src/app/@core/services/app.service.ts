...
export class AppService {
  ...
  GetSubSections(sectionId: string, typePlace: string, main: number, sub: number): Observable<ApiResponse<SubSectionsData>> {
    return this.http.get<ApiResponse<SubSectionsData>>
    (`${env.ApiUrl}/ads/get-sub-sections?section_id=${sectionId}&type_place=${typePlace}&main=${main}&sub=${sub}`);
  }
